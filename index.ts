...
const handlers: Record<string, (args: any) => any> = {
  ...
  get_lead: (args) => {
    const lead = loadData().leads.find((l: any) => l.id === args.leadId);
    return lead ? { success: true, lead } : { success: false, error: "Not found" };
  },

  update_lead: (args) => {
    const data = loadData();
    const idx = data.leads.findIndex((l: any) => l.id === args.leadId);
  ...
  delete_lead: (args) => {
    const data = loadData();
    data.leads = data.leads.filter((l: any) => l.id !== args.leadId);
    saveData(data);
    return { success: true };
  },
...
// 请求处理
// ============================================================================
async function handleRequest(path: string, queries: any, body: any, rawReq?: any) {
  // 调试端点
  if (path === "/debug") {
    return { 
      statusCode: 200, 
